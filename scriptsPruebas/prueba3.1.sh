...
if [ -f "$MEM_CONFIG" ]; then
  ...
  sed -i "s/^TAM_MEMORIA=.*/TAM_MEMORIA=512/" "$MEM_CONFIG"
  sed -i "s/^TAM_PAGINA=.*/TAM_PAGINA=32/" "$MEM_CONFIG"
  sed -i "s/^ENTRADAS_POR_TABLA=.*/ENTRADAS_POR_TABLA=32/" "$MEM_CONFIG"
  sed -i "s/^CANTIDAD_NIVELES=.*/CANTIDAD_NIVELES=1/" "$MEM_CONFIG"
...
if [ -f "$KERNEL_CONFIG" ]; then
  ...
  sed -i "s/^TIEMPO_SUSPENSION=.*/TIEMPO_SUSPENSION=1000/" "$KERNEL_CONFIG"
